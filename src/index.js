...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  else if (!user.pro && user.todos.length >= 10) return response.status(403).json({error:'update the plan to create more todo'});
...
function checksTodoExists(request, response, next) {
  ...
  if (!user) return response.status(404).json({error:"This user not exist"});
